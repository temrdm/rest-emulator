...
function restEmulator(listMockConfig, options) {
    ...
    function middleware(req, res, next) {
        ...
        if ((req.body && Object.keys(req.body).length) && (preset.body && Object.keys(preset.body).length)) {
            var status = validateBody(preset.body, req.body);

            if (status === false) {
                var errorPreset = _.clone(preset, false);
                errorPreset.code = errorResponse.code;
                errorPreset.data = errorResponse.data;
